    ...
    const charactersHandler = new lambda.Function(
      ...
      "CharactersHandler",
      {
        ...
        environment: {
          ...
          // HMAC-SHA256 secret for signing/verifying character share tokens.
          // Must match the secret used by the campaigns Lambda so that
          // per-character tokens generated by the campaign view endpoint
          // verify correctly in this handler's /characters/{id}/view route.
    ...
    const usersHandler = new lambda.Function(
      ...
      "UsersHandler",
      {
        ...
        environment: {
          ...
          PATREON_REDIRECT_URI:         (isProd
                                          ? process.env["PATREON_REDIRECT_URI_PROD"]
                                          : process.env["PATREON_REDIRECT_URI_DEV"])  ?? "",
